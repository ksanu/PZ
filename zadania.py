    ...
    def get_vector(self,inverse_vocabulary):
        ...
        for word in self.preprocessing(self.text.split()):
            if word in inverse_vocabulary.keys():
                ...
                vector[inverse_vocabulary[word]] = 1    #Multiple-Bernoulli model
        return vector
...
def MyClassification(X, y, XT, yt, Klasyfikator):
    ...
    print(klasyfikator.__class__.__name__)
    print("pozytywne: " + str(pozytywne))
    print("wszystkie: " + str(wszystkie))
    print(str(float(pozytywne)/float(wszystkie) * 100) + '%')
...
#klasyfikator.fit(X,y)
MyClassification(X, y, XT, yt, naive_bayes.MultinomialNB())
MyClassification(X, y, XT, yt, naive_bayes.BernoulliNB())
MyClassification(X, y, XT, yt, svm.SVC())
